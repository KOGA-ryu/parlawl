...
| H33 | On macOS 26.5.2 with Qt 6.11, requesting the full accessibility hierarchy immediately after switching Player Explorer to Games crashes inside `QMacAccessibilityElement::accessibilitySelectedChildren`; the same coordinate-only tab switch remains running until that hierarchy request. | Screen-reader and accessibility-automation review of the game table is blocked even though ordinary pointer navigation survives. | Keep this as a release blocker for accessible distribution: reduce it to a standalone Qt table reproduction, verify the next Qt patch release, and add a native accessibility smoke check before claiming resolution. Do not hide table semantics merely to suppress the crash. | Reproduced framework/accessibility follow-up |
