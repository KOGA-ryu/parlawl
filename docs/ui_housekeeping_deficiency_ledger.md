...
| H04 | The legacy shell exposes Puzzle Info, Move List, Analysis Replay, Game Review, and player tabs behind the new Review window. | Navigation appears duplicated and the source of truth is unclear. | Give game study a dedicated Player Explorer and reserve the legacy shell for puzzle launches. | Built and focused-test verified |
...
| H08 | `Explorer` raises the legacy shell but leaves Review and all boards visible. | Explorer becomes a fourth overlapping surface instead of a workspace switch. | Hide the complete study workspace before surfacing the dedicated Player Explorer. | Built and focused-test verified |
...
| H17 | Multiple floating boards receive the same default placement. | Comparing games produces an indistinguishable stack. | Keep bounded initial placement and add explicit Tile boards, Show all, and Focus current controls for comparison. | Built and focused-test verified |
...
| H29 | The legacy shell still owned the visible Explorer panel as well as long-lived controllers. | Returning from study necessarily resurfaced the old host UI. | Move the Explorer panel and database connection into a dedicated top-level Player Explorer. Keep only non-visible orchestration in the legacy owner until application-controller extraction is justified. | Visible-host collision resolved; controller extraction remains architectural follow-up |
| H31 | Multiple open games had tabs but no window-management affordance for their independently floating boards. | Boards could be hidden behind one another and comparison required manual window arranging. | Reveal a compact comparison bar only when useful, with Tile boards, Show all, and Focus current actions. | Built and focused-test verified |
| H32 | A board hidden for focused comparison had no defined tab-switch behavior. | Selecting a different notation tab could leave the old board visible or show two boards. | In Focus current mode, every game-tab change hides non-current boards and restores only the matching board. | Built and focused-test verified |
| H33 | On macOS 26.5.2 with Qt 6.11, programmatically activating Games through the native accessibility tab action crashed inside `QMacAccessibilityElement::accessibilitySelectedChildren`; coordinate activation remained running. | Screen-reader and accessibility automation could fail while entering the primary library through a tab switch. | Make Games the dedicated Explorer's initial view and keep the table's row/column semantics intact. Retest native tab activation on the next Qt patch rather than suppressing accessibility data. | Primary path mitigated and full Games hierarchy verified; Qt tab-action follow-up remains |
| H34 | Dedicated Player Explorer initially opened on a six-card statistics dashboard. | Opening another study required an unrelated tab switch and made Explorer feel analytics-first instead of library-first. | Start dedicated Explorer on Games while retaining Overview as an optional neighboring tab. | Built and real-data verified |
| H35 | The Explorer source-status sentence exposed a truncated source digest, coverage fraction, process boundary, and engine configuration context in the main visual hierarchy. | Technical provenance competed with the game filters and table without helping routine selection. | Show a concise local/read-only evidence state and retain the full source, coverage, and configuration lineage in the tooltip. | Built and real-data verified |
...
4. `Explorer` pauses playback, saves notes/appearance, hides Review and every board, then surfaces the dedicated Player Explorer; the puzzle shell remains hidden.
...
8. With multiple games open, Tile boards gives each board a distinct on-screen position, Show all restores them, and Focus current follows game-tab selection without closing any study.
