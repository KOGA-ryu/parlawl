...
// The publication dance, the canonical encoder and the opaque-UUID check now
// live in `append_only_export.{h,cpp}` and are shared with the market journal.
...
using parlawl::storage::openDirectoryWithoutSymlinks;
    ...
    QString *errorMessage)
{
    ...
    if (batch.terminalAttempt.has_value()) {
        ...
        insert.addBindValue(sha256Hex(terminalJson));
        if (!insert.exec()) {
            rollback();
    ...
    QString *errorMessage) const
{
    ...
    if (terminalQuery.value(0).toString() != attemptId || terminalQuery.value(1).toString() != logicalAttemptId
        ...
        || terminalQuery.value(12).toString() != sha256Hex(expectedJson)) {
