...
// The canonical-JSON core, the strict parser and the generic field checkers now
// live in `libs/strict_json/`. They are shared with the market import boundary
// rather than duplicated, so a fix to Python float spelling or duplicate-key
// rejection reaches both boundaries at once.
...
using parlawl::strictjson::kMaximumMapRows;
...
using parlawl::strictjson::validateTextMap;
